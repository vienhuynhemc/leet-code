...
public class AllPathsFromSourceToTarget {
  ...
  private void findPath(int target, int currentNode, int[][] graph, List<Integer> paths, List<List<Integer>> allPaths) {
    ...
    if (currentNode == target) allPaths.add(paths);
    else for (int visitNode : graph[currentNode]) this.findPath(
        target,
        visitNode,
        graph,
        new ArrayList<>(paths),
  ...
  public static void main(String[] args) {
    ...
    System.out.println(
      allPathsFromSourceToTarget.allPathsSourceTarget(new int[][] { { 4, 3, 1 }, { 3, 2, 4 }, { 3 }, { 4 }, {} })
    );
  }
}
